...
const FormulaHeader = ({options, title, setSelectedFormula, handleSubmitData}) => {
  ...
  return (
    <div className="flex flex-col gap-1">
      <h1 className="font-medium text-teal-900 md:text-base text-sm">{title}</h1>
      <select onChange={handleChange} className="self-start py-1.5 md:text-base text-sm border-[1.5px] border-slate-300 cursor-pointer">
         {options.map(option => {
